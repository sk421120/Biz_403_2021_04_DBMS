...
/*
iolist 테이블과 Prod 테이블간에 상품으로 JOIN을 하여 NULL 값이 없는 것이 확인되었다.

1. iolist 테이블에 상품코드 칼럼을 추가하고
2. prod 테이블에서 상품코드를 가져와 iolist 테이블에 저장
3. iolist 테이블고 prod 테이블간에 상품코드를 기준으로 JOIN을 할 수 있도록 테이블 변경을 시작한다
*/
...
-- tbl_iolist의 상품코드 칼럼에 저장된 값과 tbl_product
...
-- 1. iolist 로 부터 거래처명, 대표자명 칼럼을 기준으로 중복되지 않은 데이터를 조회 >> GROUP BY
--     거래처명, 대표자명 순으로 정렬
--  projection : 기준이 되는 칼럼을 SELECT 표현   필요한 칼럼만 나타나며 전체 데이터가 출력
--      중복이 되지 않게(같은 데이터는 1번만 출력) 칼럼을 GROUP BY 묶기
...
*/
-- import 된 거래처 정보와 매입매출정보를 JOIN하여 NULL값이 있는지 확인
...
-- tbl_dept에서 거래처 코드를 조회하여 tbl_iolist의 io_dcode 칼럼에 UPDATE
UPDATE tbl_iolist IO
SET IO.io_dcode = (
    SELECT D.d_code FROM tbl_dept D
        WHERE D.d_name = IO.io_dname AND IO.io_dceo = D.d_ceo
);
