...
-- 여러개의 데이터를 동시에 INSERT하기 다른 테이블로 부터 데이터
INSERT ALL
...
--  학생테이블의 모든데이터를 나열하고, 학과테이블에서 일치하는 데이터를 가져와서 연관하여 보여라
CREATE VIEW view_학생정보
AS (
SELECT ST.st_num 학번, ST.st_name 학생이름, ST.st_dcode 학과코드, DP.dp_name 학과명, DP.dp_dprof 담당교수, ST.st_grade 학년, ST.st_tel 전화번호, ST.st_addr 주소
FROM tbl_student ST
...
-- 학과코드 = 학과명은 항상 같은 값이 되므로 학과코드, 학과명으로 GROUP BY를 하면 학과별로 묶음이 이루어진다
...
SELECT SC.sc_num 학번, ST.st_name 이름, ST.st_dcode 학과코드, ST.st_tel 전화번호,
    ST.sc_kor 국어, SC.sc_eng 영어, SC.sc_math 수학, (SC.sc_kor + SC.sc_eng + SC.sc_math) 총점,
...
SELECT 학과코드, 학과명, SUM(총점), ROUND(AVG(평균),0)
FROM view_성적일람표
GROUP BY 학과코드, 학과명
ORDER BY 학과코드;
