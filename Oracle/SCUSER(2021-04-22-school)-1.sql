...
-- 위에서 생성한 테이블에는 데이터를 추가하려고 할때
-- 이름 데이터가 없어도 데이터가 정장적으로 추가가 되어버린다
-- 같은 학번의 데이터가 이미 추가되어 있어도 또 다시 추가가 되어버립ㄴ다
-- 이런식으로 데이터가 계속 추가된다면 전체 데이터 신뢰성에 문제가 될 것이다
-- DBMS에서는 table(Entity)를 설계할 때 이러한 오류를 방지하기 위하여 Table을 생성할 때
-- "제약조건"을 설정하여 데이터가 INSERT 되지 못하도록 하는 기능이 있다.
-- 작성된 table을 삭제하고 다시 제약조건을 설정하여 생성하자

DROP TABLE tbl_student;

-- 1. 학생의 이름은 데이터가 반드시 있어야 한다 (Not Null)
-- 2. 학번은 절대 중복되면 안된다. (유일성)
...
-- 학번 컬럼이 NOT NULL 되어있어서 INSERT 불가
INSERT INTO tbl_student(st_num, st_dept, st_name)
VALUES('사회과학', '이몽룡');

-- st_num 칼럼이 UNIQUE인데 이미 존재하는 00001 학번으로 데이터를 추가하려고 하니 문제가 있어서 INSERT 불가
...
-- 기본키 칼럼(PRIMARY KEY)
-- 데이터를 조회(SELECT) 할 때 st_num 칼럼을 기준으로 조회를 하면 반드시 원하는 데이터 1개만 보여지는 조건을 만족하게 하는 칼럼
-- 제약조건이 반드시 UNIQUE 하면서 NOT NULL 이어야 한다.
-- 기본키는 제약조건에 UNIQUE 와 NOT NULL 을 같이 설정해야 하는데 DBMS에서는 기본키 제약조건을 설정하는 키워드가 별도로 있다
CREATE TABLE tbl_student (
