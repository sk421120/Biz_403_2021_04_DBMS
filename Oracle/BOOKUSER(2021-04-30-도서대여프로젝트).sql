...
SELECT B.bk_isbn AS ISBN, B.bk_title AS 도서명,
...
CREATE VIEW view_도서정보 AS (
    SELECT B.bk_isbn AS ISBN, B.bk_title AS 도서명,
        C.cp_title AS 출판사명, C.cp_ceo AS 출판사대표,
        A.au_name AS 저자명, A.au_tel AS 저자연락처,
        B.bk_date AS 출판일, B.bk_pages AS 페이지,
        B.bk_price AS 가격
    FROM tbl_books B
        LEFT JOIN tbl_company C
            ON B.bk_ccode = C.cp_code
...
-- Table JOIN한 결과로 생성된 VIEW는 읽기전용,      INSERT, UPDATE, DELETE를 실행할 수 없다.

SELECT * FROM view_도서정보;

-- 도서정보 엑셀 데이터 수정했음 (작가 NULL값 없이)
