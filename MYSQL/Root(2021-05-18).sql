...
-- 		접근이 된다
-- MySQL : DataBase가 schema가 되고 모든 데이터의 저장공간 이된다
...
-- 192.168.0.* 현재 공유기에 공통으로 연결된
-- PC에서 MySQL Server에 접근하라
CREATE USER gbUser@'192.168.0.%';
-- 현재 공유기에 공통으로 연결된 PC 에서 접근할때 모든 권한을 부여하겠다
GRANT ALL privileges ON *.* TO 'gbUser'@'192.168.0.%';

-- 5.7 버전에서 user 비번변경하기
SELECT * FROM user;
UPDATE USER SET password=password('1234') WHERE user='gbUser';

-- MySQL 8.x 에서 비변변경하기
ALTER USER 'gbUser'@'localhost' identified WITH mysql_native_password BY '12345';
flush privileges;

SELECT * FROM USER WHERE User='gbUser';
