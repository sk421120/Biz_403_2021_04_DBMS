...
-- 사용자와 연관 없이 기본적으로 사용할 DataBase를 지정하여 사용할 준비
use MYSQL;
-- 현재 접속한 DataBase(mysql)에 있는 모든 table을 보여달라
SHOW tables;
-- myDB 라는 DataBase( 데이터 저장소 ) 생성
...
SHOW TABLES;
-- MySQL에서는 일련번호와 관련된 칼럼에 AUTO_INCREMENT 옵션을 설정하면 INSERT 할때 값을 지정하지 않아도
...
-- 도서 가격이 만원 이상 2만원 이하인 데이터
SELECT * FROM tbl_books WHERE bk_price BETWEEN 10000 AND 20000;

-- 도서명에 '왕' 문자열이 있는 데이터
SELECT * FROM tbl_books WHERE bk_title LIKE '%왕%';

-- Java 등 코딩에서 중간 문자열 검색
-- oracle	:	 '%' || ' 왕'  || '%'
SELECT * FROM tbl_books WHERE bk_title LIKE CONCAT('%', '왕','%');

-- 날짜 칼럼의 앞에 4글자만 보여라
SELECT LEFT(bk_date,4) FROM tbl_books;
SELECT * FROM tbl_books WHERE LEFT(bk_date,4) = '2018';
-- 전체 데이터를 날짜 순으로 보여라
SELECT * FROM tbl_books ORDER BY bk_date; 

-- 도서명을 역순으로 정렬하여 보여라
SELECT * FROM tbl_books ORDER BY bk_title DESC;

-- 처음 3개의 데이터만 보여라
SELECT * FROM tbl_books LIMIT 3;

-- 4(0부터 시작)번째 데이터 부터 2개
-- 게시판 등 코딩에서 pagination 을 구현할 때 사용하는 방법
SELECT * FROM tbl_books LIMIT 3,2;
